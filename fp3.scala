...
object fp3:
  ...
  def allEqual(xs: List[Int]): Boolean =
    ...
    xs match {
      case Nil => true
      case y::Nil => true
      case y::ys => if y == ys.head then allEqual(ys) else false
    }
  ...
  def removeDupes2[X](xs: List[X]): List[(Int, X)] ={
    ...
    //throw UnsupportedOperationException()
     xs match{
       case Nil => Nil
       case y::ys => removeDupes2Aux(ys, y, 1)
     }
  }
  def removeDupes2Aux[X](ys:List[X], y:X, count:Int):List[(Int,X)]= {
      ys match{
        case Nil => (count,y)::Nil
        case z::zs => if(z==y) then removeDupes2Aux(zs,z,count+1) else (count,y)::removeDupes2Aux(zs,z,1)
      }    
  }   
  ...
  def splitAt[X](n: Int, xs: List[X]): (List[X], List[X]) =
    ...
    //throw UnsupportedOperationException()
      (n,xs) match{
        case(_,Nil) => (Nil,Nil)
        case (n,_) if (n<0) => (List(),xs)
        case (n,y::ys) => { val(a,b) = splitAt(n-1,ys) 
                            if (n>0) then (y::a,b) else (a,y::b)}
    }
