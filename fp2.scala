...
object fp2:
  ...
  def joinTerminateRight(xs: List[String], term: String): String =
    // TODO: Provide definition here.
    //throw UnsupportedOperationException()
    foldRight(xs, "", (x : String, ys : String) => x + term + ys)
  ...
  def joinTerminateLeft(xs: List[String], term: String): String =
    // TODO: Provide definition here.
    //throw UnsupportedOperationException()
    foldLeft(xs, "", (x:String, ys: String) => x + ys + term)
  ...
  def firstIndexNumGreaterThanAux(a:Int, xs: List[Int], index : Int) : Int = {
	  xs match {
	    	case Nil => throw new java.util.NoSuchElementException()
	    	case y::ys if(y >= a) => index
	    	case _::ys => firstIndexNumGreaterThanAux(a, ys, index + 1)
	    }
	}
